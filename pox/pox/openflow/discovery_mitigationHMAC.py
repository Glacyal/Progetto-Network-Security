...
f1=open("GenerazionePacchettoLLDPMitigazioneHMAC.txt", 'w')
f2=open("ProcessamentoPacchettoLLDPMitigazioneHMAC.txt", 'w')
...
class LLDPSender (object):
  ...
  def _create_discovery_packet (dpid, port_num, port_addr, ttl):
    ...
    h = hmacEncryption(port_id.id, chassis_id.id, ttl.ttl, hmacKey)
    sysdesc.payload = ('dpid:' + hex(int(dpid))[2:] + ';' + h).encode()     #[2:] per togliere 0x davanti il dpid
      ...
      if t.tlv_type == pkt.lldp.SYSTEM_DESC_TLV:
        ...
        for line in t.payload.decode().split('\n'):
          if line.startswith('dpid:'):
            hmac = line.split(';')
            if hmac[1][:64] != hmacEncryption(lldph.tlvs[1].id, lldph.tlvs[0].id, lldph.tlvs[2].ttl, hmacKey):
              log.error("L'HMAC presente nel pacchetto non corrisponde a quello generato in base ai campi di riferimento.")
              end_time=time.time()
              print("Fine processamento", end_time-start_time)
